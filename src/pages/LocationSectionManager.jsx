...
//   // If it's just a path like "wedding-section5/filename.png"
...
//         console.log("Image URL from DB:", data.image_url); // Debug log
//         console.log("Full image URL:", getImageUrl(data.image_url)); // Debug log
...
//         // Refetch to get the updated data with proper image URL
...
//                 placeholder="Or enter image URL or storage path (e.g., wedding-section5/filename.png)"
//               />
//             </div>
...
//                   alt="Preview"
//                   onError={(e) => {
//                     console.error("Image failed to load:", sectionData.image_preview || sectionData.image_url);
...
//               After upload, the image path will be: wedding-section5/filename.png
//             </p>
//           </div>
//         </div>
...
//                   alt="Preview"
//                   onError={(e) => {
//                     console.error("Preview image failed to load:", sectionData.image_preview || sectionData.image_url);
...
export default function LocationSectionManager() {
  ...
  const fetchSectionData = async () => {
    try {
      ...
      if (result.success && result.data) {
        const data = result.data;
        // The backend returns full URLs, so use them directly
        setSectionData({
          title: data.title || "Location",
          subtitle: data.subtitle || "Our venues",
          description: data.description || "Our stunning venue sets the scene for exceptional wedding celebrations with elegant gardens and luxurious facilities. We invite you to explore our ceremony spaces, reception halls, bridal suites, cocktail terraces, and so much more.",
  ...
  const saveSection = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setSaved(true);
        setTimeout(() => setSaved(false), 3000);
        await fetchSectionData(); // Refetch to get updated data with full URLs
      } else {
        setError(result.message || "Error saving section");
        if (result.errors) {
          const errorMessages = Object.values(result.errors).flat().join(", ");
