...
export default function RoomBlocksHeroManager() {
  ...
  const fetchSectionData = async () => {
    try {
      const result = await apiRequest("/wedding-room-blocks/hero", "GET");
      console.log("Fetched hero data:", result);
      
      if (result.success && result.data) {
        // Handle both field names (background_image or image_url)
        const imageValue = result.data.background_image || result.data.image_url || "";
        setSectionData({
          title: result.data.title || "",
          image_url: imageValue,
          image_file: null,
          image_preview: getImageUrl(imageValue),
        });
  ...
  const saveSection = async () => {
    ...
    try {
      ...
      if (sectionId) {
        ...
        result = await apiRequest(`/admin/wedding-room-blocks/hero/${sectionId}`, "POST", submitData, token, true);
      } else {
        result = await apiRequest("/admin/wedding-room-blocks/hero", "POST", submitData, token, true);
      }
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-bold">Room Blocks Hero Section</h2>
