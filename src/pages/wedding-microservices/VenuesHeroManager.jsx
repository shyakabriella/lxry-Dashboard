...
export default function VenuesHeroManager() {
  ...
  const fetchSectionData = async () => {
    try {
      const result = await apiRequest("/wedding-venues/hero", "GET");
      ...
      if (result.success && result.data) {
        ...
        setSectionData({
          ...
          image_file: null,
          image_preview: getImageUrl(data.background_image),
        });
  ...
  const saveSection = async () => {
    ...
    if (!sectionData.image_file && !sectionData.background_image) {
    ...
    if (sectionData.image_file) {
      submitData.append("background_image", sectionData.image_file);
      setUploadingImage(true);
    } else if (sectionData.background_image && !sectionData.background_image.startsWith("blob:")) {
      submitData.append("background_image", sectionData.background_image);
    }
    ...
    try {
      if (sectionId) {
        ...
        result = await apiRequest(`/admin/wedding-venues/hero/${sectionId}`, "POST", submitData, token, true);
      } else {
        result = await apiRequest("/admin/wedding-venues/hero", "POST", submitData, token, true);
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await apiRequest(`/admin/wedding-venues/hero/${sectionId}`, "DELETE", null, token);
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-bold">Venues Hero Section</h2>
        ...
        <div className="space-y-4 bg-white p-4 sm:p-6 rounded-xl border shadow-sm">
          ...
          <div>
            ...
            <div className="flex gap-2 items-center">
              ...
              <input
                value={sectionData.background_image && !sectionData.background_image.startsWith("blob:") ? sectionData.background_image : ""}
