...
//   const handleSave = () => {
//     localStorage.setItem("wedding_prime_apartment", JSON.stringify(sectionData));
//     setHasChanges(false);
//     setSaved(true);
...
//       title: "Prime Luxury Apartment Living",
//       subtitle: "Discover Modern Apartments Designed For Comfort, Elegance And Everyday Living",
//       description: "Experience refined apartment living with premium finishes, serene surroundings, and thoughtful design.",
//       imageUrl: "https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?w=800&h=500&fit=crop",
//       amenities: [
//         "Premium finishes throughout",
//         "Serene surroundings",
...
//                 />
//                 <button onClick={() => removeAmenity(idx)} className="px-3 bg-red-500 text-white rounded">×</button>
//               </div>
//             ))}
...
export default function PrimeApartmentSectionManager() {
  ...
  const saveSection = async () => {
    ...
    try {
      if (sectionId) {
        result = await apiRequest(`/admin/wedding/section3/apartment/${sectionId}`, "PUT", submitData, token, true);
      } else {
  ...
  const handleReset = () => {
    if (sectionId) {
      // If exists, reload from database
      fetchSectionData();
    } else {
      // Reset to defaults
      setSectionData({
